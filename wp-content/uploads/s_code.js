...
var s_account="farheapblog"
var s=s_gi(s_account)

/************************** CONFIG SECTION **************************/
/* You may add or alter any code config here. */
s.dynamicAccountSelection=true
s.dynamicAccountList="farheapblog=overnightprints.com"
...
s.linkInternalFilters="javascript:,www.overnightprints.com,overnightprints.com,www.paypal.com,paypal.com"
...
function linkCode(specificErrorCode) {
 var s=s_gi('farheapblog');
 s.linkTrackVars='prop4,prop8,eVar23,events';
 s.linkTrackEvents='event5';
 s.prop4='Upload Error';
 s.prop8=specificErrorCode;
 s.eVar23=specificErrorCode;
 s.events='event5';
 s.tl(this,'o',specificErrorCode);
}

function s_doPlugins(s) {

	if (s.eVar9)
		s.transactionID=s.eVar9=s.purchaseID;
		
	if(!s.campaign)
		s.campaign=s.getQueryParam('CJPID');
	
	if(!s.campaign)
		s.campaign=s.getQueryParam('CID');
	
	s.channelManager('cmpid');
	if(!s.campaign && s._referrer !=''){
		if(s._channel=="Natural Search")	
			s.campaign=s.eVar22=s._partner+"-"+s._channel+"|||"+s._keywords;  // set evar22 regardless of s.campaign status
		else
		s.campaign=s.eVar22=s._channel+"|"+s._referrer;
	}else 
		s.campaign="Direct";
	
	
    s.clickThruQuality('s_kwcid','event2','event3'); 
	s.eVar16=s.getTimeParting('h','-8',new Date().getFullYear()); // Set hour
	s.eVar17=s.getTimeParting('d','-8',new Date().getFullYear()); // Set day

    if(s.getQueryParam('s_kwcid'))
        s.pageURL=s.manageQueryParam('s_kwcid',1,1);
		
		
	s.server=window.location.host.toLowerCase(); // domain
	s.server=replaceAll(s.server,'www.','');

	s.prop1=window.location.href.toLowerCase(); // page url
	s.pageName="D=g"; // page url
	s.pageName=replaceAll(s.pageName,'http://overnightprints.com/blog/','');
	s.prop2=document.title; // page title
	s.eVar20=s.crossVisitParticipation(s.campaign,'s_ev20','90','20','>','purchase');	//cross visit participation expires in 90 days or at purchase
	s.eVar21=s.crossVisitParticipation(s.campaign,'s_ev21','365','20','>','event50');	//cross visit participation expires in 365 days
	//s.eVar20=s.crossVisitParticipation(s.campaign,'s_ev22','30','5','>','event2',1); // this was a test setup
...
s.visitorNamespace="farheap"
s.trackingServer="farheap.122.2o7.net"
