	...
	class wpUpdaterPps {
		...
		static public function getInstance($pluginDir, $pluginFile = '', $pluginSlug = '') {
			...
			if(!isset($instances[ $instKey ])) {
				$instances[ $instKey ] = new wpUpdaterPps($pluginDir, $pluginFile, $pluginSlug);
			}
		...
		public function checkForPluginUpdate($checkedData) {
			...
			if(class_exists('framePps') && framePps::_()->getModule('license') && !framePps::_()->getModule('license')->getModel()->isExpired()) {
		...
		public function prepareRequest($action, $args) {
			...
			return array(
				...
				'user-agent' => $this->_getUserAgentHash(). '/' . $wp_version . '; ' . PPS_SITE_URL. ';'. $this->getIP()
			);	
		}
