...
class sendgridModelPps extends modelPps {
    ...
    private $_MODE   = null;//MCRYPT_MODE_CBC;
     ...
     public function getLists() {
         ...
         $listsRes = $this->makeGetRequest('contactdb/lists');

         if($listsRes && is_array($listsRes)) {
             foreach($listsRes as $list) {
                 foreach($list as $listsub) {
                     $listsDta[ $listsub['id'] ] = $listsub['name'];
                 }
             }
     ...
     public function makeRequest($address, $data = array()) {
         ...
         curl_setopt($session,CURLOPT_USERPWD, $this->getApiUsername().':'.$this->getApiPassword());
     ...
     public function makeGetRequest($address, $data = array()) {
         $res = array();
     ...
     public function makeSendRequest($address, $data = array()) {
         $res = array();
                 ...
                 if(!empty($lists)) {
                     if($this->isLoggedIn()) {
                         if(!$validateIp || $validateIp && framePps::_()->getModule('subscribe')->getModel()->checkOftenAccess(array('only_check' => true))) {
                             ...
                             $addData = json_encode(array('email' => $email,'name' => $name,'subscription_list' => 'diyfails'));
                             $addData2 = '['.$addData.']';
                             $res = $this->makeRequest('contactdb/recipients', $addData2);
                             if(!$res)
                                     return false;
                             $template_id = 'TEMPLATETOUSE';
                             $js = array(
                                   'sub' => array(':name' => array('Nonesuch')),
                             ...
                             $params = array(
                                 'to'        => $email,
                                 'toname'    => "",
                                 'from'      => "leader@diyfails.com",
                                 'fromname'  => " MYName",
                                 'subject'   => "MYWELCOMESUBJECT",
                                 'text'      => "DEFAULTTEXT",
                                 'html'      => "DEFAULTHTML",
                                 'x-smtpapi' => json_encode($js),
                               );

                             $res2 = $this->makeSendRequest('mail.send',$params);
                             if(!$res2)
                                     return false;
